...
use super::{QueueStore, StoreError, entry_id};
...
fn put_preserves_arrival_order() {
    ...
    let first = store.put(request("first"), 0, 1000).expect("put first");
    let second = store.put(request("second"), 0, 1001).expect("put second");
    let third = store.put(request("third"), 0, 1002).expect("put third");
...
fn put_samples_flutter_within_bounds() {
    ...
    for i in 0..50 {
        let entry = store
            .put(request(&format!("body {i}")), 240, 1000 + i)
            .expect("put entry");
        assert!(entry.flutter_seconds <= 240);
    }
    let zero = store.put(request("plain"), 0, 2000).expect("put plain");
...
fn bump_moves_entry_to_head() {
    let dir = TempDir::new().expect("tempdir");
    let store = open_store(&dir);
    let a = store.put(request("a"), 0, 1000).expect("put a");
    let b = store.put(request("b"), 0, 1001).expect("put b");
    let c = store.put(request("c"), 0, 1002).expect("put c");
...
fn unknown_ids_are_rejected(#[case] op: fn(&QueueStore) -> super::Result<()>) {
    let dir = TempDir::new().expect("tempdir");
    let store = open_store(&dir);
    store.put(request("a"), 0, 1000).expect("put a");
...
fn schedule_starts_immediately_when_never_posted() {
    ...
    let store = open_store(&dir);
    store.put(request("a"), 0, 1000).expect("put a");
    store.put(request("b"), 0, 1001).expect("put b");
...
fn entries_survive_reopen() {
    ...
    let a = first.put(request("a"), 0, 1000).expect("put a");
    drop(first);
