...
man pages generated by the workspace build scripts, and rely on the shared
`linux-packages` helper to produce `.rpm` and `.deb` artefacts for each binary
without invoking `nfpm` directly in the workflow.
...
matching man page. The workflow then invokes the `linux-packages` composite
action directly to generate the transient `nfpm` manifest and package the
staged artefacts in one pass. This keeps the workflow lean—only the shared
actions ever execute `nfpm`—and ensures the generated `.deb` and `.rpm` files
are ready to upload. Finally, `softprops/action-gh-release` publishes the
generated packages to a draft GitHub Release, preserving the previous
workflow's release review gate.

Runtime dependencies are injected into the packages through the `deb-depends`
and `rpm-depends` inputs exposed by the `linux-packages` action. The daemon
packages require `bash`, `systemd`, and `shadow-utils` so the installation
scripts can create the service account and manage the unit with `systemctl`.
The CLI package does not add any additional runtime dependencies.
