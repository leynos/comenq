...
man pages generated by the workspace build scripts, and call `nfpm` to emit
`.rpm` and `.deb` artefacts for each binary.
...
action compiles the workspace with the correct target toolchain, while a
follow-up step regenerates the `nfpm` manifest so that the daemon packages
include the systemd unit, configuration template, and lifecycle scripts stored
under `packaging/linux`. The `linux-packages` composite action installs `nfpm`,
enabling the workflow to rebuild the packages with the richer manifest before
uploading them. Finally, `softprops/action-gh-release` publishes the generated
packages to a draft GitHub Release, preserving the previous workflow's release
review gate.
