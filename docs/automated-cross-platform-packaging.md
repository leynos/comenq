...
# The `goreleaser-rust` plugin handles building the binaries and
# cross-compiling to multiple targets automatically.
builds:
  - id: comenq
    binary: comenq
    main: ./crates/comenq
...
`packaging/shared/config.toml` is sufficient and will be picked up by the
archive.
